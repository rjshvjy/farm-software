// https://github.com/rjshvjy/farm-software/blob/main/app/receipt/ReceiptEntry.tsx
// app/receipt/ReceiptEntry.tsx
// ---------------------------------------------------------------------------
// FARM & HOME ACCOUNTS — RECEIPT ENTRY (money in). Stage A, §17.6 item 1.
//
...
import { saveReceipt, type ReceiptLine, type SaveReceiptResult } from "./actions";
...
export default function ReceiptEntry({
  ...
  const [lastResult, setLastResult] = useState<SaveReceiptResult | null>(null);
  ...
  function buildPayload(): ReceiptLine[] {
  ...
  async function onSave() {
    ...
    setLastResult(null);
    setSaving(true);
    const result = await saveReceipt(buildPayload());
    setSaving(false);
  ...
  function helpFor(key: string | null): string {
    switch (key) {
      case "date":
        return 'Receipt date, DD/MM/YYYY — or just "19" for the 19th of this month, "19/6", "1907". The line below shows how it will be read.';
      case "mode": {
        const base =
  ...
  return (
    <main className="max-w-6xl mx-auto p-4 pb-24">
      ...
      <header className="flex items-baseline justify-between mb-4 gap-4 flex-wrap">
        <h1 className="text-2xl font-semibold">Voucher entry — receipt</h1>
