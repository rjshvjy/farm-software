// https://github.com/rjshvjy/farm-software/blob/main/app/sales/actions.ts
// app/sales/actions.ts
// ---------------------------------------------------------------------------
// FARM & HOME ACCOUNTS — the write path from the SALES INVOICE screen.
//
...
// money moves.
//
// saveSalesInvoice → fn_save_voucher with p_voucher_type = 'SALES'.
// The type routes the voucher into its own gap-free series (SI/26/0001) and
// tells the database this is a TRANSACTION shape (file 15): something happened
// on the estate, so the line must carry activity, farm and cost object.
//
// RENAMED FROM RECEIPT, 20-07-2026. What was built here was always a sales
// invoice — it carries the crop, the farm and the quantity. A receipt is the
// settlement that follows: money in against a party balance, no dimensions at
// all (Part 0.5). File 15 made RECEIPT a settlement shape, so this screen
// sending RECEIPT would now be refused, correctly. Everything else —
...
export type SalesInvoiceLine = {
  payment_date: string; // the DB column name; on this screen, the invoice date
...
export type SaveSalesResult =
  | {
      ok: true;
      voucher_no: string; // SI/26/nnnn — its own series (files 12, 15)
      entry_type: string;
      warnings: string[];
...
export async function saveSalesInvoice(
  ...
  const { data, error } = await supabase.rpc("fn_save_voucher", {
    p_lines: lines,
    p_voucher_type: "SALES", // file 15: SI/ series, TRANSACTION shape,
                             // at least one money-in line
  });
